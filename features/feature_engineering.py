...
def add_rolling_features(df):
    ...
    for w in windows:
        ...
        df[f"aqi_roll_mean_{w}"] = df["us_aqi"].rolling(w).mean()
...
def add_future_targets(df):
    ...
    df["aqi_t_plus_24"] = df["us_aqi"].shift(-24)
    df["aqi_t_plus_48"] = df["us_aqi"].shift(-48)
    df["aqi_t_plus_72"] = df["us_aqi"].shift(-72)
